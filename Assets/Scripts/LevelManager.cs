...
public class LevelManager : MonoBehaviour
{
    ...
    private void BuildWallColliders(GridRow[] gridRows, Transform parent)
    {
        ...
        for (int z = 0; z < gridRows.Length; z++)
        {
            for (int x = 0; x < laneCount; x++)
            {
                ...
                // Forward wall ¿ player runs into this, triggers bounce
                ...
                // Side walls ¿ block physically, no bounce
    ...
    private void SpawnWallCube(Transform parent, Vector3 localPos, Vector3 size, bool isForward)
    {
        GameObject w = GameObject.CreatePrimitive(PrimitiveType.Cube);
        w.name = isForward ? "WallForward" : "WallSide";
        w.transform.SetParent(parent);
        w.transform.localPosition = localPos;
        w.transform.localScale = size;
        w.layer = obstacleLayerIndex;

        Destroy(w.GetComponent<MeshRenderer>());
        Destroy(w.GetComponent<MeshFilter>());

        if (isForward)
            w.AddComponent<ForwardWall>();
    }
